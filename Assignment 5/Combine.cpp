...
//split sequences
    int midNumber = sequences.size() / 2;
    Vector<Vector<DataPoint>> firstHalf;
    Vector<Vector<DataPoint>> secondHalf;
    for(int i = 0; i < midNumber; i++){
        firstHalf.add(sequences[i]);
    }
    for(int i = midNumber; i < sequences.size(); i++){
        secondHalf.add(sequences[i]);
    }

//merge sequences
    Vector<DataPoint> firstCombined = combine(firstHalf);
    Vector<DataPoint> secondCombined = combine(secondHalf);
    Vector<DataPoint> result;

    int i = 0;
    int j = 0;
    while(i < firstCombined.size() && j < secondCombined.size()){
        if(firstCombined[i].weight < secondCombined[j].weight){
            result.add(firstCombined[i]);
            i++;
        }else{
            result.add(secondCombined[j]);
            j++;
        }
    }

    while(i < firstCombined.size()){
        result.add(firstCombined[i]);
        i++;
    }
    while(j < secondCombined.size()){
        result.add(secondCombined[j]);
        j++;
    }
    return result;
