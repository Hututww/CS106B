...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if(logicalSize == hashFn.numSlots()){
        return false;
    }

    int homeIndex = hashFn(elem) % hashFn.numSlots();
    int distance = 0;
    int currentIndex = homeIndex;
    string currentElem = elem;
    //elem 是函数参数，且是 const string& 类型，不能被修改
    //要创建一个新的变量来储存这个值。

    while(true){
        if(elems[currentIndex].distance == EMPTY_SLOT){
            elems[currentIndex].value = currentElem;//不能直接用elem，会出bug
            elems[currentIndex].distance = distance;
            logicalSize++;
            return true;
        }//看看目前的位置合不合适
        ...
        //小飞侠来咯，具体如下：
        /*
        假设有元素X，它的理想位置是索引5，但由于冲突，现在它已经探测到索引8
        假设索引8处已有元素Y，其理想位置是索引7（距离是1）
        那么，X会发现自己的距离比Y大，于是X会把Y挤走，自己占据Y的位置
        */
        if(elems[currentIndex].distance < distance){
            string tempElem = elems[currentIndex].value;
            int tempDistance = elems[currentIndex].distance;
            elems[currentIndex].value = currentElem;
            elems[currentIndex].distance = distance;
            currentElem = tempElem;
            distance = tempDistance;
        }
        
        currentIndex = (currentIndex + 1) % hashFn.numSlots();
        distance++;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for(int i = 0; i < hashFn.numSlots(); i++){
        int index = (homeIndex + i) % hashFn.numSlots();//当前位置
        ...
        if(elems[index].distance < i){
            return false;
        }//如果当前元素的距离比当前位置的距离小，那就没找到
        //相当于已经撞南墙啦！
    }
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    int homeIndex = hashFn(elem) % hashFn.numSlots();
    for(int i = 0; i < hashFn.numSlots(); i++){
        int index = (homeIndex + i) % hashFn.numSlots();
        if(elems[index].distance == EMPTY_SLOT){
            return false;
        }
        ...
        if(elems[index].value == elem){
            ...
            int nextIndex = (currentIndex + 1) % hashFn.numSlots();
            while(elems[nextIndex].distance > 0 ){
                elems[currentIndex].value = elems[nextIndex].value;
                elems[currentIndex].distance = elems[nextIndex].distance - 1;
...
void RobinHoodHashTable::printDebugInfo() const {
    cout<<"--------debug高手，来咯--------"<<endl;
    cout<<"表的大小："<<hashFn.numSlots()<<endl;
    cout<<"元素个数: "<<logicalSize<<endl;
    cout<<"负载因子: "<<(double)logicalSize / hashFn.numSlots()<<endl;
    ...
    for(int i = 0; i < hashFn.numSlots(); i++){
        ...
        if(elems[i].distance == EMPTY_SLOT){
            ...
            cout<<elems[i].value<<"(距离"<<elems[i].distance<<")"<<endl;
        }
    }
}
