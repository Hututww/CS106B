...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    for(int i = 0; i < numSlots; i++){
        int currIndex = (index + i) % numSlots;
        
        if(elems[currIndex].type == SlotType::FILLED 
            && elems[currIndex].value == elem){
            return false;
        }

        if(elems[currIndex].type == SlotType::TOMBSTONE 
            && firstTombstone == -1){
            firstTombstone = currIndex;
        }//记录墓碑，马上统一操作

        if(elems[currIndex].type == SlotType::EMPTY){
            if (firstTombstone != -1){
                currIndex = firstTombstone;
            }
            elems[currIndex].value = elem;
            elems[currIndex].type = SlotType::FILLED;
            numElems++;
            return true;
        }
    }//注意最后一种情况，如果全是墓碑的话那就挑最前面的墓碑开始用！
    if(firstTombstone != -1){
...
bool LinearProbingHashTable::remove(const string& elem) {
    int hash = hashFn(elem);
    for(int i = 0; i < numSlots; i++){
        int currIndex = (hash + i)%numSlots;
        if(elems[currIndex].type == SlotType::EMPTY){
            return false;
        }
        if(elems[currIndex].type == SlotType::FILLED 
            && elems[currIndex].value == elem){
            elems[currIndex].type = SlotType::TOMBSTONE;
            numElems--;
            return true;
        }
    }
...
void LinearProbingHashTable::printDebugInfo() const {
    ...
    cout<<"有"<<numElems<<"个元素"<<endl;
    cout<<"有"<<numSlots<<"个槽"<<endl;
    cout<<"哈希槽的情况："<<endl;
