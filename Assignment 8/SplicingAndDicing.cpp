...
void deleteNucleotides(Nucleotide* dna) {
    ...
    Nucleotide *helper = dna;
    while(helper != nullptr){
        Nucleotide *next = helper->next;
        delete helper;//在地址里删东西，而不是只删指针
        helper = next;
    }
}
...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide *leader = new Nucleotide;
    leader->value = str[0];
    leader->prev = nullptr;

    Nucleotide *helper = leader;
    for(int i = 1; i < str.size(); i++){
        Nucleotide *current = new Nucleotide;
        current->value = str[i];
        current->prev = helper;
        helper->next = current;
        helper = current;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    for(Nucleotide *startPlace = dna; startPlace != nullptr; startPlace = startPlace->next){
        Nucleotide *dnaHelper = startPlace;
        Nucleotide *targetHelper = target;//初始化
        
        while(dnaHelper != nullptr && targetHelper != nullptr 
            && dnaHelper->value == targetHelper->value){
            dnaHelper = dnaHelper->next;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide *startPlace = findFirst(dna, target);
    Nucleotide *targetRunner = target;
    Nucleotide *endPlace = startPlace;//_init_

    if(startPlace == nullptr){
        return false;
    }

    while(targetRunner->next != nullptr){
    ...
    //if在表中开切
        Nucleotide* beforeMatch = startPlace->prev;
        Nucleotide* afterMatch = endPlace->next;

        beforeMatch->next = afterMatch;
        if(afterMatch != nullptr){
            afterMatch->prev = beforeMatch;
        }
    }

    //释放切出来的片段的内存
    startPlace->prev = nullptr;
    endPlace->next = nullptr;
    deleteNucleotides(startPlace);
